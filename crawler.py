...
def get_all_product_by_url (headers: dict, url: str, ward: str ,limitPage: int = 10) -> pd.DataFrame:
    ...
    try:
        while(len_prod>0 and page <= limitPage):
            try:
                ...
                for i in range(0, len_prod-1):
                    ...
                    #description
                    description_value = house_container.find_all('div', 'product-content')
                    if (len(description_value) > 0 ):
                        description.append(description_value[0].text)
                    else: 
                        description.append("")
            
            except:
                print("fail to crawl data at ward {0} page {1}".format(ward, page))
            # sleep
        ...
        print('fail to begin crawl data')
